...
def main(args):
    ...
    base_ds = 0 # this is only used if evaluating with main.py, but for the moment we are not going to get there
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint_{}.pth'.format(datetime.today().strftime('%_%m_%d__%H_%M_%S'))]
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt".format(datetime.today().strftime('%_%m_%d__%H_%M_%S'))).open("a") as f:
