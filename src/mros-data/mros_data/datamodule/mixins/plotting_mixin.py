...
class PlottingMixin:
    def plot_signals(self, idx: int, channel_names: List[str] = None, ax: Optional[Axes] = None, predicted_events = None):
        ...
        if predicted_events != None:
            fig, ax = plot_data(
                data, events, fs, channel_names=channel_names, title=f"{record} | No. real events: {len(events)} | No. predicted events: {len(predicted_events)}", ax=ax, predicted_events = predicted_events)

        else:
            fig, ax = plot_data(
                data, events, fs, channel_names=channel_names, title=f"{record} | No. real events: {len(events)}", ax=ax, predicted_events = predicted_events)
